...
def tiled_matmul(a, b, block_size=None, executor=None, backend="auto", out=None):
    ...
    if b.ndim > 2:
        ...
        # Avoid expanding 1D 'a' to 3D which causes gufunc signature mismatch with out buffer
        if a.ndim == 1:
            with np.errstate(all='ignore'):
                return np.matmul(a, b, out=out)
        ...
        # If both are batched but have mismatched batch dimensions, fall back
        if a.ndim > 2 and a_batch != b_batch:
            with np.errstate(all='ignore'):
                return np.matmul(a, b, out=out)

        try:
            B_total = int(np.prod(b_batch))
            a_reshaped = a.reshape(B_total, a.shape[-2], a.shape[-1]) if a.ndim > 2 else a
            b_reshaped = b.reshape(B_total, b.shape[-2], b.shape[-1])

            out_shape = list(b_batch) + [a.shape[-2] if a.ndim > 1 else 1, b.shape[-1]]
            if a.ndim == 2:
                out_shape = list(b_batch) + [a.shape[0], b.shape[-1]]
            ...
            for i in range(B_total):
                a_slice = a_reshaped[i] if a.ndim > 2 else a
                futures.append(executor.submit(np.matmul, a_slice, b_reshaped[i], out=res_reshaped[i]))
