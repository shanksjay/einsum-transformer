...
def tiled_matmul(a, b, block_size=None, executor=None, backend="auto", out=None):
    ...
    if (backend == "auto" and HAS_MLX) and (hasattr(a, 'item') or hasattr(b, 'item')):
        if not hasattr(a, 'item'): a = mx.array(a)
        if not hasattr(b, 'item'): b = mx.array(b)
        return mx.matmul(a, b)
    ...
    if b.ndim > 2:
        ...
        if np.prod(batch_shape) < 32 and M_inner < 32:
            use_parallel = False
        ...
        try:
            ...
            a_flat = np.broadcast_to(a, list(batch_shape) + [M_inner, K_inner]).reshape(B_total, M_inner, K_inner)
            b_flat = np.broadcast_to(b, list(batch_shape) + [K_inner, N_inner]).reshape(B_total, K_inner, N_inner)
            res_flat = res.reshape(B_total, M_inner, N_inner)

            if executor is not None and use_parallel:
                futures = []
