...
def tiled_matmul(a, b, block_size=None, executor=None, backend="auto", out=None):
    ...
    if b.ndim > 2:
        ...
        # Calculate operations considering batched dimensions
        try:
            batch_shape = np.broadcast_shapes(a.shape[:-2], b.shape[:-2])
            M_inner = a.shape[-2]
            N_inner = b.shape[-1]
            batch_size = int(np.prod(batch_shape))
            total_ops = float(batch_size) * M_inner * K * N_inner
        except ValueError:
            with np.errstate(all='ignore'):
                return np.matmul(a, b, out=out)
        ...
        # Reshape to chunk batch dimensions
        try:
            res_flat = res.reshape(batch_size, M_inner, N_inner)
            if a.ndim == b.ndim and a.shape[:-2] == b.shape[:-2]:
                a_flat = a.reshape(batch_size, M_inner, K)
                b_flat = b.reshape(batch_size, K, N_inner)
            else:
                a_bc = np.broadcast_to(a, list(batch_shape) + [M_inner, K]).reshape(batch_size, M_inner, K)
                b_bc = np.broadcast_to(b, list(batch_shape) + [K, N_inner]).reshape(batch_size, K, N_inner)
                a_flat, b_flat = a_bc, b_bc
        ...
        try:
            ...
            if out is not None and res is out:
                if not np.shares_memory(res_flat, out):
    ...
    # Standard non-batched b implementation
