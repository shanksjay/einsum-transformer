...
def tiled_matmul(a, b, block_size=None, executor=None, backend="auto", out=None):
    ...
    if backend == "cupy" and HAS_CUPY:
        is_cp_a = hasattr(a, 'get')
        is_cp_b = hasattr(b, 'get')
        cp_a = a if is_cp_a else cp.asarray(a)
        cp_b = b if is_cp_b else cp.asarray(b)
        if hasattr(out, 'get'):
    ...
    if b.ndim > 2:
        ...
        a_flat = np.broadcast_to(a, list(batch_shape) + [M_inner, K]).reshape(-1, M_inner, K)
        b_flat = np.broadcast_to(b, list(batch_shape) + [K, N]).reshape(-1, K, N)
        res_flat = res.reshape(-1, M_inner, N)
