...
def tiled_matmul(a, b, block_size=None, executor=None, backend="auto", out=None):
    ...
    if b.ndim > 2:
        ...
        out_shape_tuple = tuple(list(batch_shape) + [M_inner, N_inner])
        ...
        try:
            a_flat = a.reshape(B_total, M_inner, K) if a.ndim > 2 else a
        ...
        def compute_batch_chunk(start_idx, end_idx):
            with np.errstate(all='ignore'):
                for idx in range(start_idx, end_idx):
                    a_slice = a_flat[idx] if a.ndim > 2 else a
                    np.matmul(a_slice, b_flat[idx], out=res_flat[idx])
